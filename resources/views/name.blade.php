    ...
    <main class="container">
        ...
        <form method="post" action="{{ route('name.store') }}">
            ...
            <div>
                ...
                <input type="text" name="name" class="input" placeholder="Enter name" id="name" autocomplete="off"
                    value="{{ old('name') }}" />
                @error('name')
            ...
            <div>
                ...
                <input type="text" name="color" class="input" placeholder="Enter color" id="color" autocomplete="off"
                    value="{{ old('color') }}" />
                @error('color')
                <span class="error" style="color: var(--accent-1);">{{ $message }}</span>
                @enderror
                    ...
                    <td>
                        <div class="btn-container">
                            ...
                            <form action="{{ route('name.destroy', $nameColor->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-default" onclick="return confirm('Alert! Sure you want to delete this item?');">
                                    Delete
                                </button>
                            </form>
                        </div>
